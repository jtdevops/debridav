...
class TorBoxWebDavClient(
    ...
    private val logger = LoggerFactory.getLogger(TorBoxWebDavClient::class.java)

    // Note: TorBox WebDAV endpoint - verify actual endpoint with TorBox documentation
    private val webdavBaseUrl = "https://webdav.torbox.app"

    override suspend fun listFiles(folderPath: String): List<WebDavFile> {
        return try {
            val url = "$webdavBaseUrl${normalizePath(folderPath)}"
            logger.debug("Listing files from TorBox WebDAV: $url")
  ...
  <D:prop>
    ...
    <D:resourcetype/>
            ...
            val response = httpClient.request(url) {
                ...
                header(HttpHeaders.Authorization, "Bearer ${torBoxConfiguration.apiKey}")
                header("Depth", "1")
            ...
            if (response.status.value == 207) { // Multi-Status
                ...
                logger.warn("Unexpected response status from TorBox WebDAV: ${response.status}")
                emptyList()
            }
        } catch (e: Exception) {
            logger.error("Error listing files from TorBox WebDAV", e)
            emptyList()
        }
    }
    ...
    override suspend fun getFileInfo(filePath: String): WebDavFile? {
        return try {
            ...
            logger.debug("Getting file info from TorBox WebDAV: $url")
...
<D:propfind xmlns:D="DAV:">
  <D:prop>
    ...
    <D:resourcetype/>
            ...
            logger.error("Error getting file info from TorBox WebDAV", e)
            null
        }
    }
