 ...
 * This logger can be configured independently to enable DEBUG logging for STRM file access
 * while keeping the rest of the application at INFO level.
 * 
 * Configuration:
 * - application.properties: logging.level.io.skjaere.debridav.resource.StrmFileAccessLogger=DEBUG
 * - Docker env: LOGGING_LEVEL_IO_SKJAERE_DEBRIDAV_RESOURCE_STRMFILEACCESSLOGGER=DEBUG
 */
object StrmFileAccessLogger {
    ...
    fun logContentComputed(file: String, provider: String?, contentType: String, content: String) {
        logger.debug("STRM_CONTENT_COMPUTED: file={}, provider={}, contentType={}, content={}", 
            file, provider ?: "null", contentType, content)
    }
    
    fun logContentSent(file: String, range: String?, contentLength: Int) {
        logger.debug("STRM_CONTENT_SENT: file={}, range={}, contentLength={}", 
            file, range ?: "full", contentLength)
    }
    
    fun logContentLengthQueried(file: String, length: Long) {
        logger.debug("STRM_CONTENT_LENGTH_QUERIED: file={}, length={}", file, length)
    }
}
