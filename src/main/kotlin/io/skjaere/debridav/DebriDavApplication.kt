...
class DebriDavApplication : SpringBootServletInitializer() {
    ...
    fun logMaintenanceEndpoints(
        ...
        return CommandLineRunner {
            ...
            logger.info("Debrid Content by Provider Endpoints:")
            logger.info("  GET     $baseUrl/api/maintenance/content")
            ...
            logger.info("          (Delete all files for non-IPTV provider, e.g. PREMIUMIZE, REAL_DEBRID)")
            logger.info("  DELETE  $baseUrl/api/maintenance/content/IPTV/{iptvProvider}")
            logger.info("          (Delete all files for IPTV provider, e.g. provider1, mega)")
            logger.info("          Note: DELETE requires maintenance API key via header (X-Api-Key) or query (apiKey).")
            logger.info("          The key is single-use and is NOT generated at startup.")
            logger.info("          On first DELETE without a valid key, a new key is generated and logged (WARN/ERROR).")
            logger.info("          Copy the key from the logs and use it for your next DELETE request.")
            logger.info("")
